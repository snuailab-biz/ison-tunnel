...
from IsonAI.ultralytics.yolo.data.augment import LetterBox
from IsonAI.ultralytics.yolo.data.utils import IMG_FORMATS, VID_FORMATS
from IsonAI.ultralytics.yolo.utils import ROOT, is_colab, is_kaggle, ops
from IsonAI.ultralytics.yolo.utils.checks import check_requirements
from config import logger

logger_detector = logger.bind(detector="detector")
import json
#/home/ljj/workspace/ison-dev/configure/field/cam3/params.json
...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            logger_detector.info(f'{st}Success ✅ ({self.frames[i]} frames of shape {w}x{h} at {self.fps[i]:.2f} FPS)')
        ...
        if not self.rect:
            logger_detector.warning('WARNING ⚠️ Stream shapes differ. For optimal performance supply similarly-shaped streams.')
        ...
        while cap.isOpened() and n < f:
            ...
            if n % self.vid_stride == 0:
                ...
                if success:
                    ...
                    # im = cv2.resize(im, dsize=(0, 0), fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR)
                    transform_im =  self.cams[i].transform_calib(im)
                    self.imgs[i] = transform_im
                else:
                    logger_detector.warning('WARNING ⚠️ Video stream unresponsive, please check your IP camera connection.')
...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # im0 = cv2.resize(im0, dsize=(0, 0), fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR)
            if im0 is None:
